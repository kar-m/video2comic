...
def cartoonify(frame, STYLE_API):
    ...
    print("Processing completed, fetching image...")
...
def create_comic_pdf(images, output_pdf):
    """
    Create a PDF with one image per page from an array of Pillow images using ReportLab.

    Args:
        images: List of PIL Image objects
        output_pdf: String path for the output PDF file
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for idx, img in enumerate(images):
            ...
            if aspect > max_height / max_width:  # Tall image
            ...
            y = (a4_height - pdf_height) / 2

            # Save image to temporary file
            temp_path = os.path.join(temp_dir, f"temp_img_{idx}.jpg")
            img.save(temp_path, "JPEG", quality=95)

            # Draw image on the page
            try:
                c.drawImage(temp_path, x, y, width=pdf_width, height=pdf_height)
                c.showPage()
            except Exception as e:
                print(f"Error adding image {idx} to PDF: {e}")
                continue
...
def get_pdf(video_path, OPENAI_API, GEMINI_API, STYLE_API, output_path):
    ...
    for frame, desc in frames:
        ...
        print("Drawing text bubble...")
        img = draw_text_bubble(model_yolo, frame, img, desc)
        print("Text bubble drawn!")
    ...
    # take out slashes of video path
    output_path = f"comic_output_{video_path.split('/')[-1].split('.')[0]}.pdf"

    print("Creating final PDF and images...")
    create_comic_pdf(final_imgs, output_path)
    print("Final PDF and images Created!")
